...
function glUpdateBegin(name) {
    _GL.recording = _GL.recordings.get(name);
}

function glUpdateEnd() {
    gl.bindBuffer(gl.ARRAY_BUFFER, _GL.recording.vertexBuffer);
    const points = new Float32Array(_GL.points);
    gl.bufferData(gl.ARRAY_BUFFER, points, gl.STATIC_DRAW);

    gl.bindBuffer(gl.ARRAY_BUFFER, _GL.recording.normalBuffer);
    const normals = new Float32Array(_GL.normals);
    gl.bufferData(gl.ARRAY_BUFFER, normals, gl.STATIC_DRAW);
